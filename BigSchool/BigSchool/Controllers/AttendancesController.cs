    ...
    public class AttendancesController : ApiController
    {
        ...
        public IHttpActionResult Attend(Course attendenceDto)
        {
            ...
            if(context.Attendances.Any(p => p.Attendee == userID && p.CourseID == attendenceDto.Id))
            {
                ...
                //xóa thông tin khóa học đã đăng ký tham gia trog bảng attendances
                context.Attendances.Remove(context.Attendances.SingleOrDefault(p => p.Attendee == userID && p.CourseID == attendenceDto.Id));
                context.SaveChanges();
                return Ok("cancel");
            }
